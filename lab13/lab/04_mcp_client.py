...
class MCPManager:
    ...
    async def __aenter__(self):
        for url in self.servers.values():
            ...
            read, write, session_id = await self._stack.enter_async_context(streamable_http_client(url))
...
async def make_llm_request(prompt: str) -> str:
    mcp_servers = {
        "plot_line": "http://localhost:8004/mcp",
    }
    result_callbacks = {"get_line_plot": [save_fig]}
    ...
    async with MCPManager(mcp_servers) as mcp:
        ...
        for _ in range(10):
            ...
            for tool_call in response.tool_calls:
                ...
                for callback in result_callbacks[func_name]:
                ...
                messages.append(
                    {
                        ...
                        "name": func_name,
                        "content": "Saved plot to image.png",
                    }
                )
...
if __name__ == "__main__":
    prompt = (
        "Plot a line chart using the provided data. "
        "A: [5,2,3,4,8]. B: [1,4,3,2,5,6]."
        "Give it a title 'Some Title' and show the legend."
    )
